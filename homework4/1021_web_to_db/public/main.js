...
R.list = function(posts) {
  ...
  <p><a href="#tst">input Sqlite code</a></p>
...
R.new2 = function() {
    return R.layout('inut sqlite code', `
<h1>inut sqlite code</h1>
<form>
<p><input type="text" placeholder="Sqlcode" name="body"></p>
<p><input id="saveCode" type="button" onclick="R.saveCode()" value="Save"></p>
</form>
`)
}
...
R.saveCode = async function() {
    let saveCode = document.querySelector('#saveCode').value
    let r = await window.fetch('/tst', {
        body: JSON.stringify({ saveCode: saveCode }),
        method: 'POST',
        headers: {
            'Content-Type': 'application/json'
        }
